import { getBibleBook } from './bible-library.ts';
...
function buildQuestionPool(bookConfig: BookMasteryBook): MasteryQuestion[] {
  const bibleBook = getBibleBook(bookConfig.bookId);
  if (!bibleBook) return [];
...
export function getBookMastery(bookId: string): BookMasteryBook | undefined {
...
export function getMasteryQuestionPool(bookId: BookMasteryBookId): MasteryQuestion[] {
  ...
  const config = getBookMastery(bookId);
  if (!config) return [];
  const questions = buildQuestionPool(config);
...
export function buildMasteryRound(bookId: BookMasteryBookId, mode: MasteryMode, seed = Date.now()): MasteryQuestion[] {
  const eligible = getMasteryQuestionPool(bookId).filter((question) => mode === 'extended' || question.tier === 'core');
