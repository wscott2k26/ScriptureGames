...
import { GENESIS_TRIALS } from '../src/genesis-season.ts';
...
for (const book of BOOK_MASTERY_BOOKS) {
  ...
  for (const mode of ['core', 'extended'] as const) {
    const round = buildMasteryRound(book.id, mode, 20260802);
...
for (const [topic, questions] of Object.entries(QUIZ_QUESTIONS)) {
  for (const [index, question] of questions.entries()) {
    assert.ok(question.verse, `${topic} question ${index + 1} is missing a Scripture reference.`);
    assert.ok(passageLocationFromReference(question.verse), `${topic} question ${index + 1} has an unopenable reference: ${question.verse}`);
  }
}

for (const trial of GENESIS_TRIALS) {
  for (const [index, question] of trial.questions.entries()) {
    assert.ok(question.reference, `${trial.id} question ${index + 1} is missing a Scripture reference.`);
    assert.ok(passageLocationFromReference(question.reference), `${trial.id} question ${index + 1} has an unopenable reference: ${question.reference}`);
  }
}
