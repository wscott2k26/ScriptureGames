...
## Current Gate
GitHub Actions is disabled at the account level. The reconstruction workflow cannot run until GitHub Support restores Actions access. Until then, use the verified local reconstruction process and do not delete the encoded payload from GitHub.